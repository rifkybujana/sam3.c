...
static void test_text_encoder_load(void)
{
	...
	for (int i = 0; i < TEST_N_LAYERS; i++) {
		...
		ASSERT(te.layers[i].qkv_w != NULL);
		ASSERT_EQ(te.layers[i].qkv_w->dims[0], 3 * TEST_WIDTH);
		ASSERT_EQ(te.layers[i].qkv_w->dims[1], TEST_WIDTH);
		ASSERT(te.layers[i].qkv_b != NULL);
		ASSERT_EQ(te.layers[i].qkv_b->dims[0], 3 * TEST_WIDTH);
