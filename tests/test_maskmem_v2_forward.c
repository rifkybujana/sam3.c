/*
 * tests/test_maskmem_v2_forward.c - Maskmem v2 forward smoke test.
 *
 * Loads the tracker_v2 maskmem backbone from models/sam3.1.sam3 and
 ...
 *
 * Key types:  sam3_tracker_v2, sam3_v2_maskmem
...
int main(void)
{
	if (access(MODEL_PATH, F_OK) != 0) {
		printf("test_maskmem_v2_forward: SKIP (%s missing)\n",
		       MODEL_PATH);
		return 0;
	}
	...
	int mask_dims[] = {1, TEST_MASK_HW, TEST_MASK_HW,
			   SAM3_V2_MULTIPLEX_IN_CHANNELS};
	int pix_dims[]  = {1, TEST_PIX_HW, TEST_PIX_HW, SAM3_V2_HIDDEN_DIM};
	...
	struct sam3_tensor *out = sam3_v2_maskmem_forward(
	...
	ASSERT_EQ(out->dims[3], SAM3_V2_HIDDEN_DIM);
	...
	printf("test_maskmem_v2_forward: PASS "
