/*
 * src/backend/cpu/kernels/cpu_add_bf16.c - BF16 elementwise add kernel
 *
 * Implements element-wise addition for bf16 tensors with broadcasting.
...
/*
 * check_broadcast_bf16 - Check if b broadcasts onto a: a is [M, N], b is [N].
 *
 * Returns the inner dimension N, or 0 if shapes match exactly,
 * or -1 on error. Does not check dtype.
 */
static int check_broadcast_bf16(const struct sam3_tensor *a,
...
static void add_bf16_scalar(const uint16_t *a, const uint16_t *b,
			    uint16_t *out, int broadcast_n,
			    int start, int end)
{
	if (broadcast_n <= 0) {
...
static void add_bf16_parallel_fn(void *arg, int task_id, int n_tasks)
{
	struct binop_par_ctx_bf16 *ctx = (struct binop_par_ctx_bf16 *)arg;
	int total, start, end;
...
#if SAM3_HAS_NEON
	add_bf16_neon(ctx->a, ctx->b, ctx->out,
		      ctx->broadcast_n, start, end);
#else
	add_bf16_scalar(ctx->a, ctx->b, ctx->out,
			ctx->broadcast_n, start, end);
#endif
}

/*
 * cpu_kernel_add_bf16 - Element-wise add for bf16 tensors with broadcasting.
 *
				    ...
				    struct sam3_threadpool *pool)
{
	if (node->n_inputs < 2 || !node->inputs[0] || !node->inputs[1] ||
	    !node->output) {
		sam3_log_error("add_bf16: NULL tensor");
		return SAM3_EINVAL;
	}

	if (node->inputs[0]->dtype != SAM3_DTYPE_BF16 ||
	    node->inputs[1]->dtype != SAM3_DTYPE_BF16) {
		sam3_log_error("add_bf16: unsupported dtype");
		return SAM3_EINVAL;
	}

	int bc = check_broadcast_bf16(node->inputs[0], node->inputs[1]);
	if (bc < 0) {
		sam3_log_error("add_bf16: shape mismatch");
		return SAM3_EINVAL;
	}

	struct binop_par_ctx_bf16 ctx = {
	...
	sam3_threadpool_parallel_for(pool, add_bf16_parallel_fn, &ctx, n_tasks);
