/*
 * src/backend/cpu/kernels/cpu_mul_f16.c - FP16 elementwise multiply kernel
 *
 * Implements element-wise multiplication for fp16 tensors with broadcasting.
...
/*
 * check_broadcast_f16_mul - Check if b broadcasts onto a: a is [M, N], b is [N].
 *
 * Returns the inner dimension N, or 0 if shapes match exactly,
 * or -1 on error. Does not check dtype.
 */
static int check_broadcast_f16_mul(const struct sam3_tensor *a,
...
static void mul_f16_neon(const _Float16 *a, const _Float16 *b, _Float16 *out,
			 int broadcast_n, int start, int end)
{
	if (broadcast_n <= 0) {
		...
		for (; i + 8 <= end; i += 8) {
			...
			vst1q_f16((__fp16 *)(out + i), vmulq_f16(va, vb));
		}
		for (; i < end; i++)
			out[i] = a[i] * b[i];
	} else {
...
static void mul_f16_scalar(const uint16_t *a, const uint16_t *b,
...
static void mul_f16_parallel_fn(void *arg, int task_id, int n_tasks)
{
	struct binop_par_ctx_f16_mul *ctx = (struct binop_par_ctx_f16_mul *)arg;
	int total, start, end;
	...
	mul_f16_neon(ctx->a, ctx->b, ctx->out,
...
static void mul_f16_parallel_fn(void *arg, int task_id, int n_tasks)
{
	...
	mul_f16_scalar(ctx->a, ctx->b, ctx->out,
		       ctx->broadcast_n, start, end);
}
...
/*
 * cpu_kernel_mul_f16 - Element-wise multiply for fp16 tensors with broadcasting.
 *
				   ...
				   struct sam3_threadpool *pool)
{
	if (node->n_inputs < 2 || !node->inputs[0] || !node->inputs[1] ||
	    !node->output) {
		sam3_log_error("mul_f16: NULL tensor");
		return SAM3_EINVAL;
	}

	if (node->inputs[0]->dtype != SAM3_DTYPE_F16 ||
	    node->inputs[1]->dtype != SAM3_DTYPE_F16) {
		sam3_log_error("mul_f16: unsupported dtype");
		return SAM3_EINVAL;
	}

	int bc = check_broadcast_f16_mul(node->inputs[0], node->inputs[1]);
	if (bc < 0) {
		sam3_log_error("mul_f16: shape mismatch");
		return SAM3_EINVAL;
	}

#if SAM3_HAS_NEON_FP16
	struct binop_par_ctx_f16_mul ctx = {
	...
	sam3_threadpool_parallel_for(pool, mul_f16_parallel_fn, &ctx, n_tasks);
